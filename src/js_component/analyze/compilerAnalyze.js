...
class analyze {
    static number(code_op) {
        return `analyze.parseNumber(${code_op.value})`;
    ...
    static string(code_op) {
        return `analyze.parseString("${code_op.value}")`;
    ...
    static boolean(code_op) {
        return `analyze.parseBboolean(${code_op.value})`;
    ...
    static variable(code_op) {
        return `analyze.parseVariable("${code_op}")`;
    ...
    static getArr(code_op) {
        ...
        return `analyze.parseGetArr(${obj},${arr})`;
    ...
    static set(code_op) {
        ...
        if (is_list(setObj)) {
            ...
            return `analyze.parseListSet(${trueObjFn},${valueFn})`;
        } else {
            //let name = analyze_entry(setObj);
            return `analyze.parseValueSet(${setObj},${valueFn})`;
        }
    ...
    static _new(code_op) {
        ...
        let classParse = analyze_entry(trueExp);
        return `analyze.parseNew(${classParse})`;
    }
...
function eval_app(operate, operands, exp) {
    ...
    console.log("eval_app____", operate)
    switch (operate.car) {
        case "original":
...
function _eval_app(operate, operands, exp, exp_env) {
    //console.log("xxxxxxxxx",exp)
    switch (operate.car) {
        case "original":
            let operate_fun = operate.cdr.car
...
class oldAnalyze {
    static number(code_op) {
        return `function (env) {
            return new _number(${code_op.value})
        }`
    }

    static string(code_op) {
        return `function (env) {
            return new _string("${code_op.value}")
        }`
    }
    static boolean(code_op) {
        return `function (env) {
            return new _boolean(${code_op.value})
        }`
    }
    ...
    static lambda(code_op) {
        ...
        let ananlyzed_body = this._sequences_analyze(body)
        return `function (env) {
            let ananlyzed_body=${ananlyzed_body};
            let args= list.parseLiteral(${args});
            return new list("compound", new lambdaBase(args, ananlyzed_body, env));
        }`
    }
    ...
    static _class(code_op) {
        ...
        return `function (env) {
            ...
            let ananlyzed_constructor_body=${ananlyzed_constructor_body};
            let constructor_args=list.parseLiteral(${constructor_args})
            let constructor_class = new _class(constructor_args, ananlyzed_constructor_body, new_env);
            return env.insert_key_value(${name},
                new list("class", constructor_class)
            )
    ...
    static app(code_op) {
        ...
        if (is_cdr_list(code_op)) {
            ...
            if (!is_list(code_op.car) && syntaxFrame.is_key_exist(code_op.car)) {
                ...
                let operands = code_op.cdr.map(analyze_entry);
                return `function (env) {
                    let operands=list.parseLiteral(${operands.literal()});
                    let true_operate = (${operate})(env);
                    let true_operands=operands.map((operand_fun) => {
                        return operand_fun(env)
                    });
